...
#include "offsets.h"
#include "xstring"
#include "C:\Users\gorzz\Documents\Huge FN Updated\reverse\lazy.h"
...
bool fillbox = true;
bool fovcirclefilled = true;
bool lineheadesp = true;
bool crosshair = true;
bool nazi = false;
...
{
	DWORD_PTR bonearray;
	bonearray = KmDrv->Rpm<DWORD_PTR>(mesh + 0x5a0);

	if (bonearray == NULL)
	{
		bonearray = KmDrv->Rpm<DWORD_PTR>(mesh + 0x5a0 + 0x10);  //(mesh + 0x5e8) + 0x5a));
	}
...
Camera GetCamera(__int64 a1)
{
	...
	__int64 v9 = KmDrv->Rpm<__int64>(v1 + 0x10); // 0x10

	FGC_Camera.FieldOfView = 80.f / (KmDrv->Rpm<double>(v9 + 0x680) / 1.19f); // 0x600
...
DWORD Menuthread(LPVOID in)
{
    while (1)
    {
        ...
        Sleep(2);
        if (MouseController::GetAsyncKeyState(VK_DELETE) & 1) {
            ShowMenu = !ShowMenu;
        }
        Sleep(2);
    }
...

{
	...
	printf("  / /_/ / / / / __ `/ _ \\   / /_  /  |/ /    \n");
	printf(" / __  / /_/ / /_/ /  __/  / __/ / /|  /     \n");
	printf("/_/ /_/\\__,_/\\__, /\\___/  /_/   /_/ |_/      \n");
	printf("            /____/                           \n\n");
  Log3(skCrypt(" Press [ F2 ] To Load Huge FN\n"));
  std::cout << skCrypt("\n");
...
void xInitD3d()
{
	...
	style->WindowMinSize = ImVec2(400, 700);
...
void DrawESP() {
	...
	if (square_fov) {
		ImGui::GetOverlayDrawList()->AddRect(ImVec2(ScreenCenterX - AimFOV, ScreenCenterY - AimFOV), ImVec2(ScreenCenterX + AimFOV, ScreenCenterY + AimFOV), ImColor(255, 255, 255), 1.5f);
		bool fovcircle = false;
		bool  fovcirclefilled = false;
	}
	if (fovcircle) {
		ImGui::GetOverlayDrawList()->AddCircle(ImVec2(ScreenCenterX, ScreenCenterY), float(AimFOV), ImColor(255, 255, 255), 100.0f, 1.5f);
		bool square_fov = false;
		bool fovcircle = false;
		bool  fovcirclefilled = false;
	}
	if (fovcirclefilled) {
		...
		ImGui::GetOverlayDrawList()->AddCircle(ImVec2(ScreenCenterX, ScreenCenterY), float(AimFOV), ImColor(255, 0, 0), 100.0f, 1.5f);
		bool square_fov = false;
		bool fovcircle = false;
	}
	char dist[64];
	...
	DrawText1(10, 20, dist, &Col.purple);
...
uintptr_t pcmc = KmDrv->Rpm<uint64_t>(PlayerController + 0x328);
PlayerState = KmDrv->Rpm<DWORD_PTR>(LocalPawn + OFFSETS::PlayerState);
DWORD_PTR PlayerCameraManager = KmDrv->Rpm<DWORD_PTR>(PlayerController + 0x338);
...
uintptr_t Crrneytwep = KmDrv->Rpm<uintptr_t>(LocalPawn + 0x7f8);
...
char bisDying = KmDrv->Rpm<char>(Localplayer + 0x6d8);
char bisBot = KmDrv->Rpm<char>(PlayerState + 0x27a);
auto bIsReloadingWeapon = KmDrv->Rpm<bool>(Crrneytwep + 0x2B9);
DWORD_PTR GameState = KmDrv->Rpm<DWORD_PTR>(Uworld + 0x148);//gamestate
   ...
   // if (read<float>(CurrentActor + OFFSETS::Revivefromdbnotime) != 10) continue;
...

	{
		if (Esp)
		{
			...
			if (nazi)
			{
				DrawCrossNazi(10, ImColor(127, 255, 0));
			}
			...
			if (Esp_box)
			{
				...
				{
					// rgba(0, 255, 231, 0)
					DrawCorneredBox(Headbox.x - (CornerWidth / 2), Headbox.y, CornerWidth, CornerHeight, IM_COL32(89, 3, 247, 255), 2.5);
				}
				else {
					// rgba(0, 255, 231, 1)
					DrawCorneredBox(Headbox.x - (CornerWidth / 2), Headbox.y, CornerWidth, CornerHeight, IM_COL32(89, 3, 247, 255), 2.5);
			...
			if (lineheadesp)
			{
				...
				DrawLine(Width / 2, Height /2, HeadElvar.x, HeadElvar.y, &Col.redlol, 1.5);
			}
			...
			if (fillbox)
			{
				DrawCorneredBox(HeadElvar.x - (CornerWidth / 2), HeadElvar.y, CornerWidth, CornerHeight, IM_COL32(89, 3, 247, 255), 2.5);
			...
			DrawOutlinedText(m_pFont, dist, ImVec2(Headbox.x, Headbox.y - 35), 16.0f, IM_COL32(0, 244, 211, 255), true);
			...
			{
					DrawLine(Width / 2, Height, bottom.x, bottom.y, &Col.skyblue, 1.5);
...
void render() {
	...
	if (ShowMenu)
	{
		...
		ImGui::Begin(menu.decrypt(), &ShowMenu, ImVec2(400, 700));
		...
		ImGui::Spacing();
		ImGui::Checkbox("Filled FOV", &fovcirclefilled);
		if (fovcirclefilled)
		{
			square_fov = false;
			fovcircle = false;
		...
		ImGui::Spacing();
		ImGui::Checkbox("Middle Snaplines", &lineheadesp);
		if (lineheadesp)
		{
			Esp_line = false;
		}
		...
		ImGui::Spacing();
		ImGui::Spacing();
		ImGui::Text("Discord: https://discord.gg/e8FCBBFfez");
		ImGui::Spacing();
		ImGui::Spacing();
		ImGui::Spacing();
		ImGui::Spacing();
		...
		if (process_find(_xor_("FortniteClient-Win64-Shipping_EAC.exe"))) {
			ImGui::Text("Loaded With EAC");
		}
		if (process_find(_xor_("FortniteClient-Win64-Shipping_BE.exe"))) {
			ImGui::Text("Loaded With BE");
		}
		ImGui::Spacing();
		ImGui::Text("Base Id: %p", (void*)base_address);
		ImGui::End();
	}
